 ...
 * 所以取「**离目标最近**」的那一档（见 `axisTickCount`），实测一档落在 **17~47px** ——
 * 下沿比理想的 19.4px 还低一点，是因为引擎对数据域还会再取整一次（实测命中过 17.7px）。
 */
 ...
 * （目标 30px 时是 19~47px；引擎对数据域再取整一次，实际下沿能到 **17px**）——
 * 比默认 5 档时的 57~71px 密得多，缩放 / 平移时也不会漂。
 *
