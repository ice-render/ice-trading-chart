...
- 用 `interaction.hover.mark: false` 关掉引擎的悬停标记。默认是**整列每个系列各画一个**
  （折线是半透明白填充的圆环），一根 K 会连带 MA / BOLL 一起被圈上，白花花地盖住 K 线，
  比准星本身还抢眼。蜡烛系列自己那圈淡描边（`CandlestickSeries.drawHoverOverlay`）保留。
