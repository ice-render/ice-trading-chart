...
## 指标预热期：多取一段历史，别让副图左边空着

MACD(12,26,9) 要 **33 根**才有第一个柱值（慢线 26 + 信号线 9），BOLL(20)/EMA(26) 也要 20~25 根。
不处理的话副图左侧会空出一大块 —— 用户一眼就问「为什么 MACD 左边缺一大块」。

做法：**多生成 `WARMUP`（40）根只喂指标、不显示**（`seed()` 里生成 `count + WARMUP` 根，
`seriesCandles` 只取尾部 `count` 根画图），指标在全量上算完再用 `tail()` 裁到可见段 ——
与真实终端「多取一段历史」同一个思路。量程类（`macdRange`）按**可见段**算，副图才贴数据。
