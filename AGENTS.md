...
| 部件 | 谁画 | 位置 |
|---|---|---|
| 竖线 | **引擎**（`crosshair.axis: 'x'`） | 吸附到悬停那一根的类目中心；经联动在**三块 pane 同步出现**，是一把贯穿整摞图的时间标尺 |
| 横线 | 页面（`.hline`） | **指针所在那一块**里自由跟随；读数用那一块自己的 y 轴（在成交量 pane 上读出来就是成交量） |
| 右轴价签 | 页面（`.tag.price`） | 贴着绘图区右缘，纵坐标跟指针 |
| 时间签 | 页面（`.tag.time`） | **最下面那块**的时间轴上（时间标尺在整摞图的最下方），按标签实际宽度居中并夹在绘图区内 |

要点：

- 竖线吸附、横线自由，是刻意的搭配：竖线吸附才好和底部时间签一一对应，
  横线自由才读得到「指针所在的那个价格」。**别把横线交给引擎** —— 引擎的横线位置取的是
  悬停那一根的**收盘价**像素（吸附式），和指针位置脱钩。
