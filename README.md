...
| 拖动平移 | K 线区域可拖动（**横纵都能拖**，纵向拖即手动量程）；绘图区内光标是「可抓」的小手、拖动中变「抓住」；用户拖过之后**图表不再自动向右延伸**，并亮出「回到最新」 |
| 十字准星 | 竖线由引擎画并**贯穿三块 pane**（吸附到悬停那一根，是一把时间标尺）；横线画在**指针所在的那一块**里、读数按那一块的量纲（成交量 pane 上读到的是成交量）；右轴价签 + 底部时间签；抬头实时跟随 |
