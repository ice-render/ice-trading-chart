...
| **推荐** | **≥ 0.30.10** | 在 0.30.1 之上是**性能与外观**改进：更新流水线的增量（类目域 / 索引表、稠密轴标签、**视窗裁剪下的类目查表**）+ 两处裁剪（高亮环、直角坐标系列）+ **远景密度**（柱子按像素列聚合、类目表逐项相同时复用、类目轴上的虚拟折线）。**不影响正确性**，只是更快、更干净。滚动看盘（轴只显示窗口里那一段）**一定吃 0.30.5**：每 tick 1.6ms → 0.3ms（10 万点实测）。 |
